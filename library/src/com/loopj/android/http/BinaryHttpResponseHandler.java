...
public class BinaryHttpResponseHandler extends AsyncHttpResponseHandler {
    ...
    @Deprecated
    public void onFailure(int statusCode,Throwable error, byte[] binaryData) {
    ...
    protected void sendFailureMessage(int statusCode,Throwable e, byte[] responseBody) {
        sendMessage(obtainMessage(FAILURE_MESSAGE, new Object[]{statusCode,e, responseBody}));
    }
    ...
    protected void handleFailureMessage(int statusCode,Throwable e, byte[] responseBody) {
    ...
    protected void handleMessage(Message msg) {
        ...
        switch(msg.what) {
            ...
            case FAILURE_MESSAGE:
                response = (Object[])msg.obj;
                handleFailureMessage(((Integer) response[0]).intValue() ,(Throwable)response[1], (byte[]) response[2]);
                break;
            default:
    ...
    void sendResponseMessage(HttpResponse response) {
        ...
        try {
            ...
            sendFailureMessage(status.getStatusCode(),e, "获取失败".getBytes());
        }
       

        if(status.getStatusCode() >= 300) {
            sendFailureMessage(status.getStatusCode(),new HttpResponseException(status.getStatusCode(), status.getReasonPhrase()),"获取失败".getBytes());
        } else {
